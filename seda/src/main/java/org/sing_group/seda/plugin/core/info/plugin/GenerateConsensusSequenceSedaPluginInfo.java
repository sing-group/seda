...
public class GenerateConsensusSequenceSedaPluginInfo extends AbstractInfo {
  ...
  public static final String PARAM_CONSENSUS_BASE_HELP = longEnumStringForCli(
    ...
    cliMap(
      ...
      Arrays.stream(ConsensusBaseStrategy.values()).map(ConsensusBaseStrategy::getDescription).toArray(String[]::new)
    )
  );

  public static final String PARAM_CONSENSUS_BASE_HELP_GUI = toHtml(
    longEnumStringForGui(
      "The strategy for selecting the bases at each position that should be considered to create the consensus.",
      guiMap(
        ConsensusBaseStrategy.values(),
    ...
    ), emptyList(), Arrays.asList("minimum presence threshold"), false
  );
  ...
  public static final String PARAM_VERBOSE_HELP =
    "In protein sequences, when this option is unselected then X is used for ambiguous positions in the consensus sequence. "
      + "On the other hand, when this option is selected, then all amino acids in such positions are reported (for instance [HWY]).";
