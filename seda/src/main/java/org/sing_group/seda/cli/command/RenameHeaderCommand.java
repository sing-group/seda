...
import static org.sing_group.seda.plugin.core.RenameHeaderSedaPluginInfo.PARAM_RENAME_HELP;
...
public class RenameHeaderCommand extends SedaCommand {
  ...
  public static final StringOption OPTION_RENAME_TYPE =
    new StringOption(PARAM_RENAME_TYPE_NAME, PARAM_RENAME_TYPE_SHORT_NAME, PARAM_RENAME_HELP, true, true);
  ...
  protected TransformationProvider getTransformation(Parameters parameters) {
    ...
    HeaderTarget target = HeaderTarget.valueOf(parameters.getSingleValue(OPTION_TARGET).toUpperCase());

    if (!parameters.hasOption(OPTION_RENAME_TYPE)) {
      formattedValidationError("Rename type is mandatory");
    }

    RenameHeaderTransformationConfigurationPanel.Rename renameType =
    ...
    try {
      switch (renameType) {
        ...
        case REPLACE_INTERVAL:
          ...
          break;
        default:
          formattedValidationError("Unknown rename type, check command help to see the available options");
      }
    } catch (IllegalArgumentException exception) {
      formattedValidationError(exception.getMessage());
    }
