...
public class SequencesGroupIsoformTesterTest {
	...
	public void selectIsoformsTest() {
		...
		SequencesGroupIsoformTesterResult result =
				new SequencesGroupIsoformTester(group).test(6);

		assertEquals(4, result.getIsoformsLists().size());

		assertEquals(Arrays.asList(S1, S3), result.getIsoformsLists().get(0));
		assertEquals(Arrays.asList(S2), result.getIsoformsLists().get(1));
		assertEquals(Arrays.asList(S4, S5), result.getIsoformsLists().get(2));
		assertEquals(Arrays.asList(S6, S7, S8), result.getIsoformsLists().get(3));
	}
}
